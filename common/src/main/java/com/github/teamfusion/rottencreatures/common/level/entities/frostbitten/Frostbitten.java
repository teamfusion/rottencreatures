    ...
    public boolean doHurtTarget(Entity entity) {
        ...
        if (hurt && this.getMainHandItem().isEmpty()) {
            ...
            if (entity instanceof LivingEntity living && living.canFreeze()) {
                ...
                living.addEffect(new MobEffectInstance(RCMobEffects.FREEZE.getHolder().get(), 140 * (int)modifier), this);
            }
        }
    ...
    protected void onChangedBlock(ServerLevel level, BlockPos pos) {
        ...
        if (this.onGround()) {
            ...
            for (BlockPos blockPos : BlockPos.betweenClosed(pos.offset(-i, -1, -i), pos.offset(i, -1, i))) {
                if (blockPos.closerToCenterThan(this.position(), i)) {
                    ...
                    if (blockState2.isAir()) {
                        ...
                        if (blockState3 == FrostedIceBlock.meltsInto()
                            ...
                            && level.isUnobstructed(state, blockPos, CollisionContext.empty())) {
                            level.setBlockAndUpdate(blockPos, state);
                            level.scheduleTick(blockPos, Blocks.FROSTED_ICE, Mth.nextInt(this.getRandom(), 60, 120));
                        }
                    }
                }
            }
        ...
        if (this.isBaby()) {
            ...
            if (!this.level().isClientSide) {
                ...
                for (LivingEntity entity : entities) {
                    ...
                    if (entity.canFreeze()) {
                        float modifier = this.level().getCurrentDifficultyAt(this.blockPosition()).getEffectiveDifficulty();
                        entity.addEffect(new MobEffectInstance(RCMobEffects.FREEZE.getHolder().get(), 100 * (int)modifier), this);
                    }
                }
            }
        }
