...
public class Mummy extends SpellcasterZombie {
    ...
    private void summonScarabs(int max) {
        if (this.level instanceof ServerLevel server) {
            for (int i = 0; i <= this.random.nextInt(max); i++) {
                ...
                Scarab scarab = RCEntityTypes.SCARAB.get().create(server);
                if (scarab != null) {
                    if (this.isAncient()) scarab.setVariant(1);
                    scarab.moveTo(pos, 0.0F, 0.0F);
                    scarab.finalizeSpawn(server, server.getCurrentDifficultyAt(pos), MobSpawnType.MOB_SUMMONED, null, null);
                    server.addFreshEntity(scarab);
                }
            }
        }
    }
    ...
    class SummonScarabsGoal extends UseSpellGoal {
        ...
        protected int getCastingTime() {
            return 60; // Faster casting
        }

        @Override
        protected int getCastingInterval() {
            // Ancient mummies can summon more frequently
            return Mummy.this.isAncient() ? 260 : 340;
        }

        @Override
        protected int getCastWarmupTime() {
            return 15; // Slightly faster warmup
        }
    }
}