...
public class Immortal extends SpellcasterZombie {
    ...
    protected void customServerAiStep() {
        ...
        if ((this.tickCount + this.getId()) % 1200 == 0) {
            Holder<MobEffect> effect = MobEffects.DAMAGE_BOOST;
    ...
    public boolean doHurtTarget(Entity entity) {
        ...
        if (hurt && (this.getMainHandItem().isEmpty() || this.isDashing()) && entity instanceof LivingEntity living) {
            ...
            living.addEffect(new MobEffectInstance(RCMobEffects.CHANNELLED.getHolder().get(), 140 * (int)modifier), this);
            Zap.convertToZap(this, living);

            if (this.isDashing()) entity.hurt(this.damageSources().mobAttack(this), 16.0F);
        }
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (source == this.damageSources().lightningBolt()) {
            this.restoreHealth();
            return false;
        } else if (source == this.damageSources().cactus() || Objects.equals(source.getMsgId(), "arrow")) {
            return false;
        } else {
            if (this.hasPower()) {
        ...
        if (recentlyHit) {
            ...
            if (this.hasPower()) {
                if (this.random.nextFloat() <= 0.75F) {
                    stack.enchant(this.random.nextBoolean() ? enchantments.get(Enchantments.CHANNELING).orElseThrow() : enchantments.get(Enchantments.RIPTIDE).orElseThrow(), 1);
                }
    ...
    class DashingGoal extends UseSpellGoal {
        ...
        protected void performSpellCasting() {
            ...
            this.immortal.level().playSound(null, this.immortal, SoundEvents.TRIDENT_RIPTIDE_3.value(), SoundSource.PLAYERS, 1.0F, 1.0F);
