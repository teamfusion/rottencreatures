...
public class Zap extends Zombie {
    ...
    protected void addBehaviourGoals() {
        ...
        this.goalSelector.addGoal(1, new ZapFollowLeaderGoal(this, Immortal.class, 1.25));
    ...
    public static void convertToZap(LivingEntity entity, LivingEntity target) {
        if (entity.level() instanceof ServerLevel level) {
            if (target instanceof Zombie zombie && !target.getType().is(RCEntityTypeTags.IMMORTAL_CANNOT_CONVERT)) {
                Zap zap = zombie.convertTo(RCEntityTypes.ZAP.get(), true);
                if (zap != null) {
                    zap.finalizeSpawn(level, level.getCurrentDifficultyAt(zap.blockPosition()), MobSpawnType.CONVERSION, null);
                    if (!entity.isSilent()) {
